...
class Display extends CI_Controller
{
    ...
    public function index($name_url = null)
    {
        // $data = array();
        // var_dump($data);
        $data['display'] = $this->m_display->getJoin(
            'plasma',
            'type_plasma',
            '*',
            'name as type_name',
        ...
        $data['images'] = $this->m_display->getAllJoin(
            'plasma',
            'type_plasma',
            'image_slide',
        ...
        $data['list_doctor_of_dutty'] = $this->m_display->getAllJoin(
            'plasma',
            'doctor_of_dutty',
            'doctor',
            '*',
            'doctor_id',
            'name as doctor_name',
            'plasma.id = doctor_of_dutty.plasma_id',
            'doctor_of_dutty.doctor_id = doctor.id',
            'plasma.url="' . $name_url . '"',
            'plasma.is_active = 1'
        );
        $data['list_nurse_of_dutty'] = $this->m_display->getAllJoin(
            'plasma',
            'nurse_of_dutty',
            'nurse',
            '*',
            'nurse_id',
            'name as nurse_name',
            'plasma.id = nurse_of_dutty.plasma_id',
            'nurse_of_dutty.nurse_id = nurse.id',
            'plasma.url="' . $name_url . '"',
            'plasma.is_active = 1'
        );
        $data['list_pic_nurse'] = $this->m_display->getAllJoin(
            'plasma',
            'pic_nurse',
            'nurse',
            '*',
        ...
        $data['list_nurse_spv'] = $this->m_display->getAllJoin(
            'plasma',
            'nurse_spv',
            'nurse',
            '*',
        ...
        $is_doctor_of_dutty      = $data['display']['0']->is_doctor_of_dutty;
        ...
        if ($type_name == 'igd') {
            if (($is_doctor_of_dutty != 1) or ($is_specialist_doctor != 1) or ($is_pic_nurse != 1) or ($is_image_slide != 1)) {
                ...
                $data['list_nurse'] = $this->m_display->getAllJoin(
                    'nurse',
                    'nurse_of_dutty',
                    'plasma',
                    'name as nurse_name',
            ...
            $c_day = date('l');
            $data['rajal'] = $this->m_display->getAllJoin(
                'plasma',
                'doctor_room',
                'doctor_schedule',
                'url, is_active',
            ...
            $data['visit_time'] = $this->m_display->getAllJoinRow(
                'plasma',
                'type_plasma',
                'visit_time',
                'id as id_plasma, type_id',
        ...
        // echo 'Type = '.$type_name.' - Dokter Jaga = '.$is_doctor_of_dutty.' - Dokter Specialis = '.$is_specialist_doctor.' - Perawat ='.$is_nurse.' - Perawat Jaga ='.$is_pic_nurse;
        // redirect('home');
    }
}
